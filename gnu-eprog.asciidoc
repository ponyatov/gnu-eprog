...
00000004 t loop <1>
00000000 T sum
$ arm-none-eabi-ld -Ttext=0x0 -o sum.elf summain.o sumsub.o
...
<1> символ `loop` имеет адрес `0x4` в `sumsub.o`, и `0x28` в `sum.elf`, так как
секция `.text` из `sumsub.o` размещена сразу за секцией `.text` из `summain.o`.
...
Для увеличения заметности искуственно размести секцию `.text` по адресу `0x100`:
